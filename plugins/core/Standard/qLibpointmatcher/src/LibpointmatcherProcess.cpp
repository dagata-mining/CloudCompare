...
bool LibpointmatcherProcess::Subsample(const LibpointmatcherDialog& dlg, QString& errorMessage,  QWidget* parentWidget/*=nullptr*/, ccMainAppInterface* app/*=nullptr*/)
{	
	errorMessage.clear();
	...
	DP filteredCloud;
	filteredCloud = LibpointmatcherTools::filter(convertedCloud);
...
bool LibpointmatcherProcess::Compute(const LibpointmatcherDialog& dlg, QString& errorMessage, ccPointCloud*& outputCloud, bool allowDialogs, QWidget* parentWidget/*=nullptr*/, ccMainAppInterface* app/*=nullptr*/)
{
	errorMessage.clear();
	...
	LibpointmatcherNormals::ComputationMode normMode = dlg.getNormalsComputationMode();
	...
	if (!error)
	{
		...
		case LibpointmatcherNormals::MULTI_SCALE_MODE:
		{
			...
			if (normMode == LibpointmatcherNormals::MULTI_SCALE_MODE)
			{
			...
			normalsAreOk = LibpointmatcherNormals::ComputeCorePointsNormals(s_M3C2Params.corePoints,
			...
			if (normalsAreOk)
			{
				...
				if (normMode == LibpointmatcherNormals::HORIZ_MODE)
				{
					LibpointmatcherNormals::MakeNormalsHorizontal(*s_M3C2Params.coreNormals);
				}
				...
				else //or use external points
				{
					...
					if (!LibpointmatcherNormals::UpdateNormalOrientationsWithCloud(s_M3C2Params.corePoints,
						*s_M3C2Params.coreNormals,
						orientationCloud,
						maxThreadCount,
						&pDlg))
	...
	while (!error) //fake loop for easy break
	{
		...
		assert(normMode == LibpointmatcherNormals::VERT_MODE || (s_M3C2Params.coreNormals && corePointCount == s_M3C2Params.coreNormals->currentSize()));
		...
		//other options
		s_M3C2Params.updateNormal = (normMode != LibpointmatcherNormals::VERT_MODE);
